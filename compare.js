...
function BrowserifyUnpack(options) {
	...
	if(options.verbose) {
		console.log("\nBrowserifyUnpack parsed options: " + JSON.stringify({
...
BrowserifyUnpack.prototype.readSourceMap = function (src) {
	...
	var fromMapFile = (!fromSource && this.filepath && convertSourceMap.fromMapFileSource(src, path.dirname(this.filepath)));

	if(this.bVerbose) {
		console.log(fromSource ? "Reading source map from embedded comment" : (fromMapFile ? "Reading source map from source map URL" : "Could not load source map, check source file"));
	}
...
BrowserifyUnpack.prototype.generateFiles = function (files, originalSource, sourceMapData, toPath) {
	...
	if (this.bVerbose) {
		console.log("Creating directory: '" + baseUrl + "', sourceDir: " + this.sourceDir);
	}
	...
	files.forEach(function(file) {
		...
		try {
			...
			if(devFilePath.indexOf(":\\") > -1) {
				console.error("cannot create absolute path containing ':\\', consider using the 'relativizeOutputPath' option, the path = '" + devFilePath + "'\n");
			}
		}

		var browserifyVarName = fileOutputPath.replace(/([\/|:\\|\\|\.|\-])/g, '_');

		var browserifyUpdate = this.createUpdateEvent(fileOutputPath.replace(/([:\\|\\])/g, '/'), values(file.deps));
		...
		if (this.bVerbose) {
			console.log("\nsourceMapData '" + file.path + "' = " + sourceMapData.files[file.path]);
		}
	...
	if (this.bMap) {
		...
		if (this.bVerbose) {
			console.log("browserify.map.json generated: '" + (this.name + '/browserify.map.json') + "'");
		}
	}
...
BrowserifyUnpack.prototype.createUpdateEvent = function (eventFileEvent, externals) {
	 ...
	 for (var i in externals) {
		var externalEvent = String(externals[i]).replace(this.sourceDir, '').replace(this.rootDir, '');
